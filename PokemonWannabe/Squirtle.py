...
class Squirtle:
    ...
    def attack1(self, Enemy, cs1, cs2, *broadcast_args ):
        ...
        self.broadcast("Squirtle uses Water Gun!\n", cs1, cs2, *broadcast_args)
        power = 40
        pp = 25
    ...
    def attack2(self, Enemy, cs1, cs2, *broadcast_args):
        print("Squirtle uses Bite!")
        self.broadcast("Squirtle uses Bite!\n", cs1, cs2, *broadcast_args)
        power=60
        pp=25
        if (self.ultimatecooldown != 0):
            self.ultimatecooldown = self.ultimatecooldown - 1
        damage = self.calculatedamage(power, pp)
        Enemy.getattacked(self, damage, cs1, cs2)
        self.broadcast("\n", cs1, cs2, *broadcast_args)

    def attack3(self, Enemy, cs1, cs2, *broadcast_args):
        print("Squirtle uses Tackle!")
        self.broadcast("Squirtle uses Tackle!\n", cs1, cs2, *broadcast_args)
        power=40
        pp=20
        if(self.ultimatecooldown!=0):
            self.ultimatecooldown=self.ultimatecooldown-1
    ...
    def attack4(self, Enemy, cs1,cs2, *broadcast_args):
        ...
        if (self.ultimatecooldown == 0):
            ...
            self.broadcast("Squirtle uses Hydro Pump!\n", cs1, cs2, *broadcast_args)
            chance=random.randint(1,101)
            if(chance<acc):
                damage = self.calculatedamage(power, pp)
                print("Attack successful!")
                self.broadcast("Attack successful!\n", cs1, cs2,*broadcast_args)
                Enemy.getattacked(Pikachu, damage, cs1, cs2)
            else:
                print("Miss!")
                broadcast("Miss!\n", cs1, cs2)
        else:
